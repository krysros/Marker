...
def _extract_address(json_ld, visible_text, visible_lines, country_code):
	...
	for node in _iter_nodes(json_ld):
		...
		street = _first_not_empty(candidate.get("streetAddress"), candidate.get("street"))
		postcode = _first_not_empty(candidate.get("postalCode"), candidate.get("zipCode"))
		...
		country = _first_not_empty(candidate.get("addressCountry"), candidate.get("country"))
	...
	if not address["street"]:
		address["street"] = _extract_by_regex(_STREET_RE, text_with_lines or visible_text)
