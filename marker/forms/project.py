...
class ProjectForm(Form):
    ...
    link = StringField('Link', validators=[Length(max=2000, message='Długość nie może przekraczać %(max)d znaków')], filters=[strip_filter])
    deadline = DateField('Termin')
    stage = SelectField('Etap', choices=STAGES)
    project_delivery_method = SelectField('Sposób realizacji', choices=PROJECT_DELIVERY_METHODS)
