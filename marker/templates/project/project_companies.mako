  ...
  <table class="table table-striped">
    ...
    <tbody id="new-assoc">
      % for assoc in companies_assoc:
        <tr>
          <td>
            ${checkbox.checkbox(assoc.company, selected_ids=selected_ids('selected_companies'), url=request.route_url('company_check', company_id=assoc.company.id, slug=assoc.company.slug))}
          </td>
          <td>
            <a href="${request.route_url('company_view', company_id=assoc.company.id, slug=assoc.company.slug)}">
              ${assoc.company.name}
            </a><br/>
            <small class="text-body-secondary">${_("Created at")}: ${assoc.company.created_at.strftime('%Y-%m-%d %H:%M:%S') if assoc.company.created_at else '---'}</small><br/>
            <small class="text-body-secondary">${_("Updated at")}: ${assoc.company.updated_at.strftime('%Y-%m-%d %H:%M:%S') if assoc.company.updated_at else '---'}</small>
          </td>
          <td>
            ${assoc.company.city or "—"}<br/>
            <small class="text-body-secondary">${get_subdivision_name(assoc.company.subdivision, "—")}</small><br/>
            <small class="text-body-secondary">${get_country_name(assoc.company.country, "—")}</small>
          </td>
          <td>
