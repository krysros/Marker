...
<div class="hstack gap-2 mb-4">
  ...
  <div>${button.button('tag_map_companies', color='secondary', icon='map', tag_id=tag.id, slug=tag.slug)}</div>
</div>
...
<div class="hstack gap-2 mb-4">
  ...
  <div>${button.button('tag_export_companies', color='primary', icon='download', tag_id=tag.id, slug=tag.slug, _query={'filter': dd_filter._filter, 'sort': dd_sort._sort, 'order': dd_order._order})}</div>
</div>
