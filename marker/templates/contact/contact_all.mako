...
<div class="d-flex flex-wrap align-items-center gap-2 mb-4 pb-1">
  ...
  <div class="btn-group btn-group-sm" role="group">
    <a class="btn ${'btn-primary' if q.get('duplicates') != '1' else 'btn-outline-primary'} btn-sm" href="${request.route_url('contact_all', _query={**q, 'duplicates': None})}"><i class="bi bi-people"></i> ${_("All")}</a>
    <a class="btn btn-outline-primary btn-sm" href="${request.route_url('contact_unassigned')}"><i class="bi bi-person-x"></i> ${_("Unassigned")}</a>
    <a class="btn ${'btn-primary' if q.get('duplicates') == '1' else 'btn-outline-primary'} btn-sm" href="${request.route_url('contact_all', _query={**q, 'duplicates': '1'})}"><i class="bi bi-files"></i> ${_("Duplicates")}</a>
  </div>
</div>
