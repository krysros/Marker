...
<h2>
  ...
  <div class="float-end">
    ${button.clear('user_clear_selected_companies', icon='square', username=user.name)}
  </div>
</h2>
<hr>
...
<div class="hstack gap-2 mb-4">
  ...
  <div>${button.button('user_export_selected_companies', color='primary', icon='download', username=user.name, _query={'filter': dd_filter._filter, 'sort': dd_sort._sort, 'order': dd_order._order})}</div>
</div>
