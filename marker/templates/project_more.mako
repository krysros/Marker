...
% endif
  ...
  <td>
    ...
    <span class="badge text-bg-success" role="button" hx-get="${request.route_url('project_watched', project_id=project.id, slug=project.slug)}" hx-target="#main-container" hx-swap="innerHTML show:window:top">${project.count_watched}</span>
    % else:
