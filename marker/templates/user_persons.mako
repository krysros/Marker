...
<div class="hstack gap-2 mb-4">
  <div class="me-auto">
    <ul class="nav nav-pills">
      ...
      <li class="nav-item">
        <a class="nav-link active" aria-current="page" href="${request.route_url('user_persons', username=user.name)}">Osoby <span class="badge text-bg-secondary">${c_persons}</span></a>
      </li>
      <li class="nav-item">
...
<div class="hstack gap-2 mb-4">
  <div>${button.dropdown('user_persons', items=dropdown_sort, criterion=sort, typ='sort', title='Sortuj', username=user.name)}</div>
  <div>${button.dropdown('user_persons', items=dropdown_order, criterion=order, typ='order', title='Kolejność', username=user.name)}</div>
</div>
