    ...
    <ul class="nav nav-pills">
      ...
      <li class="nav-item">
        <a class="nav-link" href="${request.route_url('project_similar', project_id=project.id, slug=project.slug)}">
          Podobne <span class="badge text-bg-secondary"><div id="similar-projects" hx-get="${request.route_url('count_similar_projects', project_id=project.id, slug=project.slug)}" hx-trigger="tagProjectEvent from:body">${project.count_similar}</div></span></a>
        </a>
      </li>
    </ul>
  </div>
