...
<a class="btn btn-primary" role="button" href="${request.route_url(route_name, **kwargs)}">Eksportuj</a>
</%def>
...
% if request.is_authenticated and request.identity.role == 'editor':
<button type="button" class="btn btn-danger" hx-post="${request.route_url(route_name, **kwargs)}" hx-headers='{"X-CSRF-Token": "${get_csrf_token()}"}' hx-confirm="Czy jesteś pewny?">Usuń</button>
% else:
...
% if request.is_authenticated and request.identity.role == 'editor':
<button type="button" class="btn btn-danger" hx-post="${request.route_url(route_name, **kwargs)}" hx-headers='{"X-CSRF-Token": "${get_csrf_token()}"}' hx-confirm="Czy jesteś pewny?">Wyczyść</button>
% else:
<button type="button" class="btn btn-danger" disabled>${title}</button>
% endif
</%def>
