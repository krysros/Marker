...
<div class="d-flex flex-wrap align-items-center gap-2 mb-4 pb-1">
  <div class="btn-group btn-group-sm" role="group" aria-label="${_('View mode')}">
    <a class="btn btn-outline-primary" href="${request.route_url('user_selected_contacts_projects', username=user.name)}"><i class="bi bi-table"></i> ${_("Table")}</a>
    <a class="btn btn-outline-primary" href="${request.route_url('user_map_selected_contacts_projects', username=user.name)}"><i class="bi bi-map"></i> ${_("Map")}</a>
    <a class="btn btn-primary" href="${request.route_url('user_uptime_selected_contacts_projects', username=user.name)}"><i class="bi bi-globe"></i> ${_("Uptime")}</a>
  </div>
...
<table class="table table-striped">
  ...
  <tbody id="uptime-tbody">
    <%include file="user_uptime_selected_contacts_projects_rows.mako"/>
  </tbody>
</table>
</div>
