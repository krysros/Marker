...
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2">
  <h2 class="mb-0 text-nowrap flex-grow-1 flex-shrink-1">
    ...
    <span class="badge bg-secondary">
      <div hx-get="${request.route_url('user_selected_companies_count', username=user.name)}" hx-trigger="selectedCompaniesEvent from:body">${counter}</div>
    </span>
  </h2>
...
<div class="d-flex flex-wrap align-items-center gap-2 mb-4 pb-1">
  ...
  <div class="btn-group btn-group-sm" role="group" aria-label="${_('View mode')}">
    <a class="btn ${'btn-primary' if matched_route_name == 'user_selected_companies' else 'btn-outline-primary'}" href="${request.route_url('user_selected_companies', username=user.name, _query=q)}"><i class="bi bi-table"></i> ${_("Table")}</a>
  ...
  <div class="vr mx-1"></div>
  <div class="btn-group btn-group-sm" role="group" aria-label="${_('View mode')}">
    <a class="btn btn-outline-primary" href="${request.route_url('user_uptime_selected_companies', username=user.name, _query=q)}"><i class="bi bi-globe"></i> ${_("Uptime")}</a>
    <a class="btn ${'btn-primary' if matched_route_name == 'user_duplicates_selected_companies' else 'btn-outline-primary'}" href="${request.route_url('user_duplicates_selected_companies', username=user.name, _query=q)}"><i class="bi bi-files"></i> ${_("Duplicates")}</a>
    <a class="btn ${'btn-primary' if matched_route_name == 'user_nolocation_selected_companies' else 'btn-outline-primary'}" href="${request.route_url('user_nolocation_selected_companies', username=user.name, _query=q)}"><i class="bi bi-geo"></i> ${_("No location")}</a>
  </div>
  <div class="btn-group btn-group-sm" role="group" aria-label="${_('Pivot view')}">
    ...
    <a class="btn btn-outline-primary" href="${request.route_url('user_selected_companies_contacts', username=user.name)}"><i class="bi bi-people"></i> ${_("Contacts")}</a>
  </div>
  <div class="vr mx-1"></div>
