...
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2">
  ...
  <div class="d-flex flex-wrap gap-2 justify-content-md-end w-100 w-md-auto">
    ...
    ${button.a(icon='download', color='primary', title='XLSX', url=request.route_url('user_export_companies_stars', username=user.name, _query=q))}
    ${button.a(icon='download', color='outline-primary', title='ODS', url=request.route_url('user_export_companies_stars', username=user.name, _query={**q, 'format': 'ods'}))}
  </div>
</div>
<hr>
