...
<div class="hstack gap-2 mb-4 d-flex flex-wrap">
  ...
  <div class="btn-group ms-auto" role="group" aria-label="${_('View mode')}">
    ...
    <a class="btn btn-primary" href="${request.route_url('user_selected_contacts_companies', username=user.name, _query=q)}">${_("Companies")}</a>
    <a class="btn btn-outline-primary" href="${request.route_url('user_selected_contacts_projects', username=user.name, _query=q)}">${_("Projects")}</a>
  </div>
</div>
