...
from marker.views.tag import TagView, TagAISearchCache, _ai_search_cache
...
def test_company_add_ai_exceptions_logging(monkeypatch, dbsession):
    ...
    req = _make_request(dbsession, user, method="POST", post={"website": "http://test-ai.com"})
    monkeypatch.setattr("marker.views.company.company_autofill_from_website", lambda *a, **kw: {
    ...
    monkeypatch.setattr("marker.views.company.location_details", raising_location_details)
...
def test_project_add_ai_exceptions_logging(monkeypatch, dbsession):
    ...
    req = _make_request(dbsession, user, method="POST", post={"website": "http://test-ai-proj.com"})
    monkeypatch.setattr("marker.views.project.project_autofill_from_website", lambda *a, **kw: {
    ...
    def raising_location_details(*a, **kw):
        ...
        raise RuntimeError("Simulated geocode failure")
    monkeypatch.setattr("marker.views.project.location_details", raising_location_details)
...
def test_match_tags_ai_model_config_exception(monkeypatch):
    monkeypatch.setattr("marker.views.tag.get_configured_model", lambda: exec('raise ValueError("not set")'))
...
def test_match_tags_ai_two_stage_exceptions(monkeypatch):
    ...
    def empty_kw_invoke_text(prompt, model, *, fallback_model=None, retries=2, source=""):
...
def test_tag_all_with_ai_query(monkeypatch, dbsession):
    ...
    tag = _make_tag(dbsession, user, "AITag")
    transaction.commit()
...
def test_user_selected_views_and_counts(dbsession):
    ...
    contact = _make_contact(dbsession, user, company=company, project=project, name="SelectedContact")
    ...
    # Add selections
    dbsession.execute(selected_companies.insert().values(user_id=user.id, company_id=company.id))
    dbsession.execute(selected_projects.insert().values(user_id=user.id, project_id=project.id))
    dbsession.execute(selected_contacts.insert().values(user_id=user.id, contact_id=contact.id))
    dbsession.execute(selected_tags.insert().values(user_id=user.id, tag_id=tag.id))
    dbsession.execute(companies_stars.insert().values(user_id=user.id, company_id=company.id))
    dbsession.execute(projects_stars.insert().values(user_id=user.id, project_id=project.id))
...
def test_user_selected_views_parameter_combinations_and_filters(dbsession, monkeypatch):
    ...
    assert len(res3["paginator"]) == 0  # No selected contacts tags because no selected contacts exist
    ...
    monkeypatch.setattr("marker.views.user.handle_bulk_selection", lambda *a: "bulk_result")
